...
describe("Simple Queries", () => {
    ...
    it("should select app count with rating of 5 stars", async done => {
        const query = `select count(*) as count from apps where apps.rating = 5.0`;
    ...
    it("should select top 3 develepors with most apps published", async done => {
        const query = `select count(*) as count, developer from apps group by developer order by  count desc, developer limit 3`;
    ...
    it("should select count of reviews created in year 2014, 2015 and 2016", async done => {
        const query = `select count(*) as review_count, substr(date_created, 7, 4) as year from reviews 
        where substr(date_created, 7, 4) in ('2014', '2015', '2016') group by year order by review_count limit 3`;
        const result = await db.selectMultipleRows(query);
        expect(result).toEqual([
