...
describe("Simple Queries", () => {
  ...
  it(
    ...
    async done => {
      const query = `select original_title, budget, revenue from movies order by budget desc, revenue desc, original_title limit 3`;
  ...
  it(
    ...
    async done => {
      const query = `select count(*) as count, user_id from movie_ratings where rating group by user_id order by count desc limit 3 `;
      const result = await db.selectMultipleRows(query);
